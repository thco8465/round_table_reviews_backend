...
def get_game_data():
    ...
    try:
        ...
        api_url = f'https://api.twitch.tv/helix/search/categories?query={game_name}'
        print('Requesting URL:', api_url)

        response = requests.get(api_url, headers=headers)

        print('Twitch API Status Code:', response.status_code)
        print('Twitch API Raw Response:', response.text)
        ...
        if data.get('data'):
            ...
            for game in data['data']:
                ...
                cover_image = game['box_art_url'].replace('52x72', '285x380')
                results.append({'gameId': game_id, 'title': game_title, 'cover': cover_image})
